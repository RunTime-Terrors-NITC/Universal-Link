...
function displayGesture(message, isSent = true) {
    // Clear placeholder
    if (
        gestureMessagesContainer.querySelector('[style*="text-align: center"]')
    ) {
    ...
    while (gestureMessagesContainer.children.length > 20) {
        gestureMessagesContainer.removeChild(
            gestureMessagesContainer.lastChild,
        );
    }
}
...
async function initApp() {
    try {
        ...
        if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
            throw new Error(
                "getUserMedia is NOT supported. Use HTTPS or localhost.",
            );
        }
...
function createPeerConnection() {
    ...
    pc.onicecandidate = (event) => {
        if (event.candidate === null) {
            ...
            showStatus(
                "Code generated! Copy 'Your Code' and send it.",
                "ready",
            );
...
async function handleRemoteSdp() {
    ...
    try {
        ...
        log("Setting remote description: " + remoteDesc.type);
        ...
        if (remoteDesc.type === "offer") {
            ...
            // If we received an answer, connection will establish
            showStatus(
                "Answer processed. Connection establishing...",
                "waiting",
            );
        }
    } catch (e) {
        console.error(e);
        showStatus("Invalid code or error: " + e.message, "error");
        log("Error processing remote SDP: " + e.message, "error");
    }
}
